...
// mcp-prometheus binaries, one per platform, each next to its cosign Sigstore bundle.
const githubRepoSlug = "giantswarm/mcp-prometheus"
...
func newSelfUpdateCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "self-update",
		Short: "Update mcp-prometheus to the latest version",
		Long: `Checks for the latest release of mcp-prometheus on GitHub and
...
and the installed binary stays as it is.`,
		RunE: runSelfUpdate,
		// A refused update is an error message, not a usage mistake: keep
		// cobra from printing the flag help after it.
		SilenceUsage: true,
	}
}
